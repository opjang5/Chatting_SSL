...
import com.ustb.dh.DH;
import com.ustb.rc4.RC4;
...
public class MainUI extends JFrame implements UpdateUiListener,MouseListener,MouseMotionListener{
	...
	public void addFriendsList() {
		...
		Icon icon0 = new ImageIcon("img/leaf_def.gif");
	...
	class SendMsg implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if(chkBox.isSelected()){
				tmpk = "";//Get B
				String p = DH.Generateg(20);
				String g = DH.Generateg(10);
				String a = DH.Generateg(10);
				String A= DH.Momi(g,a,p);
				sc.sendMsg("ask@@"+userName+"@@"+name+"@@"+p+"@@"+g+"@@"+A);
				while(tmpk.equals(""));
				tmpk = DH.Momi(tmpk, a, p);
				sc.sendMsg("msg@@"+userName+"@@"+RC4.Encrypt(tmpk, txtMessage.getText())+"@@"+name);
			}
			else
	...
	public void message(String result) {
		...
		else if(op.equals("capk")){
			capk = (result.split("%"))[1];
		}
		else if(op.equals("ursk")){
			try {
				ursk = RSA.pubDecrypt(capk,(result.split("%"))[1]);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}
		else if(op.equals("tmpk")){
			tmpk = (result.split("%"))[1];
		}
		else if(op.equals("ask")){
			//tmpk = (result.split("%"))[1];
			String b = DH.Generateg(10);
			String p = (result.split("%"))[3];
			String g = (result.split("%"))[4];
			String A = (result.split("%"))[5];
			String B = DH.Momi(g, b, p);
			tmpk = DH.Momi(A, b, p);
			sc.sendMsg("ans@@"+(result.split("%"))[2]+"@@"+B);
		}
		else if(op.equals("ans")){
			tmpk = (result.split("%"))[2];
		}
		else if(op.equals("pri")){
			String message = "*** Private >> From "+(result.split("%"))[1]
							+" to "+(result.split("%"))[2]
							+" : "+(result.split("%"))[3]+"\n"
							+ RC4.Decrypt(tmpk, (result.split("%"))[4])+"\n";
			chatArea.append(message+"\r\n");
		}
		else if(op.equals("pub")){
			String message = "Public >> From"+(result.split("%"))[1]
							+" : "+(result.split("%"))[2]+"\n"
							+(result.split("%"))[3]+"\n";
			chatArea.append(message+"\r\n");
		}
